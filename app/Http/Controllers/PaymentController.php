...
class PaymentController extends Controller
{
    ...
    public function reportpayment(Request $request)
    {
        ...
        $data["data"] = Payment::getRepMedical($f_patient_id, $f_medical_code, $f_method, $start_date, $end_date);
        $data["code"] = MedicalTreatment::select("medical_code")->orderBy("date", "desc")->get();
        $data["pasien"] = Patients::select("name", "patient_id")->orderBy("name", "asc")->get();
        $data["filter"] = array("page" => $perpage, 
                            "f_patient_id" => $f_patient_id,
    ...
    public function cetakpayment(Request $request)
    {
        ...
        if($format == "pdf"){
            ...
            $pdf = PDF::loadview("print.payment", $data)
            ->setOptions(['defaultFont' => 'sans-serif'])->setPaper('folio', 'landscape');
            return $pdf->stream();
        }
        
        return view("print.payment", $data);
    }
}
